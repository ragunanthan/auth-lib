...
import { signUpBodyValidation, logInBodyValidation } from "./validationSchema.js";
...
const generateTokens = async (userData: any, addToken: Function) => {
  try {
...
const logout = ({ deleteExistingToken }: { deleteExistingToken:  (id : string) => Promise<{ success: boolean }>  }) => async (req: any, res: Response) => {
  try {
...
const verifyRefreshToken = (userId: string, refreshToken: string, getToken: Function) => {
...
const refreshToken = ({ getToken }: { getToken: () => Promise<{ success: boolean }> }) => async (req: any, res: Response) => {

  return verifyRefreshToken(req.user._id, req?.headers?.authorization?.split(' ')[1], getToken)
    .then(async () => {
...
const getUserDetail = ({ getUser }: { getUser: Function }) => async  (req: any, res: Response) => { 
  let { _id } = req.user;
